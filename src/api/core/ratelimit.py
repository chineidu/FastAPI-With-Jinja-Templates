    ...
    async def ais_rate_limited(
        ...
        *,
        ...
        try:
            ...
            _user_id: str = str(user_id) if len(str(user_id)) <= 8 else str(user_id)[:8] + "..."
            logger.exception(f"Rate limiting check failed (user_id={_user_id}, path={sanitized_path})")
...
async def get_rate_limiter(
    ...
    if is_limited:
        raise RateLimitError(f"Rate limit exceeded for path '{path}'. Try again in {retry_after} seconds.")
