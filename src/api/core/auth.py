...
async def get_current_user(
    ...
    client_schema: BaseClientSchema | None = client_repo.convert_DBClient_to_schema(db_client)
...
async def get_current_user_or_guest(
    ...
    client_schema: BaseClientSchema | None = client_repo.convert_DBClient_to_schema(db_client)
    if client_schema is None:
        logger.error(f"Failed to convert DBClient to ClientSchema for name: {name}, returning guest user")
        return GuestClientSchema()
...
async def authenticate_user(db: AsyncSession, username: str, password: str) -> DBClient | None:
