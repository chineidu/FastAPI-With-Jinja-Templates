...
oauth2_scheme_optional = OAuth2PasswordBearer(tokenUrl=f"{auth_prefix}/token", auto_error=False)
...
def generate_api_key(
    ...
    prefix: str = custom_prefix + "".join(secrets.choice(alphabet) for _ in range(prefix_len))
    body: str = "".join(secrets.choice(body_alphabet) for _ in range(total_len - prefix_len))
...
async def get_current_user(
    ...
    db_user = await user_repo.aget_user_by_username(username=username)
    if db_user is None:
        raise credentials_exception
...
async def get_current_user_or_guest(
    ...
    db_user = await user_repo.aget_user_by_username(username=username)
    if db_user is None:
        logger.warning(f"user not found for username: {username}, returning guest user")
        return GuestUserSchema()

    user_schema: BaseUserSchema | None = user_repo.convert_DBUser_to_schema(db_user)
    if user_schema is None:
        logger.error(f"Failed to convert DBUser to UserSchema for username: {username}, returning guest user")
        return GuestUserSchema()

    if not user_schema.is_active:
        logger.warning(f"Inactive user attempted access: {username}, returning guest user")
        return GuestUserSchema()

    return user_schema


async def authenticate_user(db: AsyncSession, username: str, password: str) -> DBUser | None:
    """Authenticate user with username and password."""
    user_repo = UserRepository(db)
    db_user = await user_repo.aget_user_by_username(username)
    if not db_user:
        return None
...
def require_scope(*required_scopes: str) -> Callable[..., Coroutine[Any, Any, None]]:
    ...
    async def _arequire_scope(
        ...
        if not all(scope in api_key.scopes for scope in required_scopes):
            logger.warning(f"API key {api_key.key_prefix} missing required scopes: {required_scopes}")
            raise HTTPError(
                status_code=status.HTTP_403_FORBIDDEN,
