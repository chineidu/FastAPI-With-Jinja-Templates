...
class BillingMiddleware(BaseHTTPMiddleware):
    ...
    def _should_bill(self, request: Request, response: Response) -> bool:
        ...
        return (response.status_code == status.HTTP_200_OK) and hasattr(request.state, "api_key_client_id")
    ...
    def _extract_billing_info(self, request: Request) -> dict[str, Any] | None:
        ...
        cost = getattr(request.state, "api_key_cost", app_settings.CREDIT_COST_PER_REQUEST)
    ...
    def _attach_billing_task(self, response: Response, billing_info: dict[str, Any]) -> None:
