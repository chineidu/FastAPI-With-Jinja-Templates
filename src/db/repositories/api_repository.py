...
class APIKeyRepository:
    ...
    async def aget_keys_by_owner(self, owner_id: int) -> list[DBAPIKey]:
        ...
        try:
            stmt = select(DBAPIKey).where(DBAPIKey.user_id == owner_id).order_by(DBAPIKey.created_at.desc())
            result = await self.db.scalars(stmt)
            return list(result.all())
        except Exception as e:
    ...
    async def acreate_api_key(self, api_key_obj: APIKeySchema) -> int:
        ...
        try:
            ...
            logger.info(f"Successfully created api_key with {db_obj.id} in the database.")
            return db_obj.id
    ...
    async def aupdate_api_key(
        ...
        for field, value in update_data.items():
            if field not in ALLOWED_FIELDS:
                logger.warning(f"Attempt to update disallowed field '{field}' on API Key {key_id}")
                continue
    ...
    async def adelete_owned_key(self, key_id: int, owner_id: int) -> bool:
        ...
        try:
            ...
            logger.error(f"Error deleting api_key id {key_id} for owner {owner_id}: {e}")
            await self.db.rollback()
            return False
...
async def aupdate_last_used_at(key_id: int) -> None:
    ...
    async for session in aget_db():
        try:
            stmt = update(DBAPIKey).where(DBAPIKey.id == key_id).values(last_used_at=func.now())
