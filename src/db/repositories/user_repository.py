...
class UserRepository:
    ...
    async def aget_user_by_external_id(self, external_id: str) -> DBUser | None:
        ...
        try:
            stmt = select(DBUser).where(DBUser.external_id == external_id).options(selectinload(DBUser.roles))
            return await self.db.scalar(stmt)
        except Exception as e:
            logger.error(f"Error fetching user by external_id '{external_id}': {e}")
            return None

    async def aget_user_by_external_ids(self, external_ids: list[str]) -> list[DBUser]:
        """Get users by their external IDs with eager loading."""
        try:
            stmt = (
                select(DBUser).where(DBUser.external_id.in_(external_ids)).options(selectinload(DBUser.roles))
            )
            result = await self.db.scalars(stmt)
            return list(result.all())
        except Exception as e:
            logger.error(f"Error fetching user by ids {external_ids}: {e}")
            return []

    async def aget_user_by_username(self, username: str) -> DBUser | None:
        """Get a user by its username with eager loading."""
        try:
            stmt = select(DBUser).where(DBUser.username == username).options(selectinload(DBUser.roles))
            return await self.db.scalar(stmt)
        except Exception as e:
    ...
    async def aget_user_by_email(self, email: str) -> DBUser | None:
        ...
        try:
            stmt = select(DBUser).where(DBUser.email == email).options(selectinload(DBUser.roles))
            return await self.db.scalar(stmt)
        except Exception as e:
    ...
    async def aget_users_cursor(
        ...
        """
        try:
            query = select(DBUser).order_by(DBUser.id.asc()).limit(limit).options(selectinload(DBUser.roles))
    ...
    async def aget_users_by_creation_time(self, created_after: str, created_before: str) -> list[DBUser]:
    ...
    async def acreate_user(self, user: UserSchema) -> int:
        ...
        try:
            db_user = DBUser(**user.model_dump(exclude={"id", "created_at", "updated_at"}))
        except Exception as e:
        ...
        try:
            ...
            logger.info(f"Successfully created user with ID {db_user.id} in the database.")
            return db_user.id
    ...
    # ----- Update operations -----
    async def aupdate_user(self, user_id: int, update_data: dict[str, Any]) -> DBUser | None:
        ...
        for field, value in update_data.items():
            if field not in ALLOWED_FIELDS:
                logger.warning(f"Attempt to update disallowed field '{field}' on user {user_id}")
                continue
    ...
    async def aupdate_credits(self, user_id: int, amount: float) -> DBUser | None:
        ...
        try:
            await self.db.commit()
            await self.db.refresh(db_user)
            logger.info(f"Adjusted credits for {user_id}: {amount:+.2f}. New Balance: {db_user.credits}")
            return db_user
    ...
    async def aassign_role_to_user(self, user_id: int, role: RoleTypeEnum) -> None:
        ...
        try:
            ...
            insert_stmt = user_roles.insert().values(user_id=user_id, role_id=db_role.id)
            await self.db.execute(insert_stmt)
            await self.db.commit()
            logger.info(f"Assigned role='{role.value}' to user_id='{user_id}'.")

        except Exception as e:
            logger.error(f"Error assigning role='{role.value}' to user_id='{user_id}': {e}")
            await self.db.rollback()
            raise e
    ...
    async def abatch_update_users(self, users: list[UserSchema]) -> None:
        ...
        existing_users = await self.aget_user_by_external_ids(external_ids)
        ...
        for user in users:
            ...
            if not existing_user:
                logger.warning(f"User with external_id {user.external_id!r} does not exist. Skipping update.")
                continue

            # Filter out fields that are None/excluded
            updated_values = {
                k: v
                for k, v in user.model_dump(exclude={"id", "created_at", "updated_at"}).items()
                if v is not None
            }
    ...
    async def aupdate_user_status(self, external_id: str, status: UserStatusEnum) -> None:
        ...
        try:
            ...
            stmt = update(DBUser).where(DBUser.external_id == external_id).values(**update_values)
            await self.db.execute(stmt)
            await self.db.commit()
            logger.info(f"Marked external_id='{external_id}' as {status.name}.")

        except Exception as e:
            logger.error(f"Error marking external_id='{external_id}' as {status.name}: {e}")
            await self.db.rollback()
            raise e
